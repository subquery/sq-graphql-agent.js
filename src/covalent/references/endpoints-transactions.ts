...
> **Note:** Base cost isÂ \`0.1\`Â credits (e.g. requests that return with statusÂ \`200\`Â but no items).

Calls with \`no-logs\` costÂ \`0.05\`Â credits/item.

The following tracing features each costÂ \`0.05\`Â credits where available on [Foundational Chains](https://goldrush.dev/chains):

- \`with-internal\`Â - includes internal transfers/transactions.
- \`with-state\`Â - includes all transaction state changes with before and after values.
- \`with-input-data\`Â - includes the transactionâ€™s input data such as the Method ID.
...
> **Note:** Response is wrapped with \`data\` field and includes \`error\`, \`error_message\`, \`error_code\` fields.

| Field | Type | Description |
|-------|------|-------------|
| \`data.updated_at\` | string | The timestamp when the response was generated. Useful to show data staleness to users. |
| \`data.items\` | array | List of response items. |
| \`data.pagination\` | object | Pagination metadata. |
| \`error\` | boolean | Whether an error occurred. |
| \`error_message\` | string | null | Error message if any. |
| \`error_code\` | number | null | Error code if any. |
