...
**IMPORTANT:** Response is an array at root level (accessed via \`data\` from API wrapper).
  ...
  "data": [
    {
      ...
      "logo_urls": {...},
      "prices": [
        {"date": "2024-01-01", "price": 2500.00, "pretty_price": "$2,500.00"},
        {"date": "2024-01-02", "price": 2550.00, "pretty_price": "$2,550.00"}
      ]
    }
  ]
}
...
**JSONPath examples:**
- \`[0]\` - First token's data
- \`[0].prices\` - All prices for first token
- \`[0].prices[*].price\` - All price values
- \`[0].prices[*].date\` - All price dates
- \`[0].contract_ticker_symbol\` - Token symbol

### Response Fields (per item in data array)
...
| \`update_at\` | string | Last update timestamp. |
| \`quote_currency\` | string | The requested quote currency eg: \`USD\`. |
| \`logo_urls\` | object | The contract logo URLs. |
| \`prices\` | array | Array of price objects with date, price, pretty_price fields. |
...
### Price Fields (each object in prices array)

| Field | Type | Description |
|-------|------|-------------|
| \`date\` | string | The date of the price capture (YYYY-MM-DD). |
...
| \`contract_metadata\` | object | Optional: Contract metadata if available. |
